<?php
	//对学生数据库的操作
	class S_usermodel extends CI_Model{
	
		//用户绑定（写入用户微信openid到数据库）
		function adduser($u_num,$openid){
			//UPDATE 表名称 SET 列名称 = 新值 WHERE 列名称 = 某值
			$sql = "UPDATE s_student SET s_wxid = '".$openid."' WHERE s_id = '".$u_num."'";
            $this->db->query($sql);
		...
		public function queryuser($u_name,$u_num,$u_id){
			...
			$sql = "SELECT * FROM s_student WHERE s_name = '".$u_name."' AND s_id =' ".$u_num."'";
		...
		function deluser(){
			...
			$sql = "DELEATE * FROM s_student WHERE s_wxid = '".$openid."'";
			$query = $this->db->query($sql);
			if($query->result())
				return true;
			else
